...
const ClinicOnboarding: React.FC = () => {
  const { setShowOnboarding, themeClasses } = useApp();
      ...
      <div className="w-full max-w-4xl space-y-8">
          ...
          {STEPS.map((step) => (
            <div key={step.id} className="relative z-10 flex flex-col items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
                currentStep >= step.id ? `${themeClasses.primary} text-white shadow-lg` : 'bg-white border-2 border-slate-200 text-slate-300'
              }`}>
                <i className={`fa-solid ${step.icon} text-xs`}></i>
              </div>
              <span className={`text-[10px] font-black uppercase tracking-widest ${
                currentStep >= step.id ? themeClasses.text : 'text-slate-400'
              }`}>{step.title}</span>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl border border-slate-100 min-h-[500px] flex flex-col">
          <div className="flex-1">
              ...
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" placeholder="e.g., Riverside Wellness Hub" className={`w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ${themeClasses.ring} transition-all`} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Primary Specialty</label>
                    <select className={`w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ${themeClasses.ring} transition-all`}>
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <input type="text" placeholder="123 Clinical Way, Suite 400..." className={`w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ${themeClasses.ring} transition-all`} />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-4">
                  <div className="flex gap-4">
                    <input type="email" placeholder="colleague@clinic.com" className={`w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ${themeClasses.ring} transition-all`} />
                  ...
                  <div className="border border-slate-100 rounded-2xl p-4 divide-y divide-slate-50">
                    <div className="py-3 flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full ${themeClasses.secondary} ${themeClasses.text} flex items-center justify-center text-[10px] font-bold`}>JW</div>
            ...
            {currentStep === 3 && (
              <div className="space-y-8 animate-in slide-in-from-right-4 duration-300 text-center">
                ...
                <div className={`p-8 ${themeClasses.primary} rounded-[2.5rem] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl ${themeClasses.shadow}`}>
                  <div className="text-left space-y-1">
                    <h3 className="text-xl font-bold">Professional Clinic</h3>
                    <p className="text-white/80 text-sm">Unlimited clients • Up to 5 therapists • Branding</p>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-black">$249/mo</p>
                    <p className="text-white/60 text-xs">7-day free trial included</p>
                  </div>
                </div>
                <button className={`text-xs font-bold ${themeClasses.text} uppercase tracking-widest hover:underline`}>Change Plan</button>
              </div>
            )}
          ...
          <div className="pt-8 border-t border-slate-50 flex justify-between items-center">
            ...
            <button 
              onClick={next} 
              className={`px-10 py-4 ${themeClasses.primary} text-white rounded-2xl font-black shadow-xl ${themeClasses.shadow} hover:opacity-90 transition-all flex items-center gap-2`}
            >
