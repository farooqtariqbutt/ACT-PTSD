...
const ClientDashboard: React.FC = () => {
  ...
  // ── Schedule lock logic ───────────────────────────────────────────────────
  const preference = (
    userProfile?.schedulePreference || user!.schedulePreference || 'MonThu'
  ...
  const skillsProgress = useMemo(() => {
    ...
    return [
      { label: 'Defusion',          value: Math.floor(base * 0.8) + 12, color: 'bg-indigo-500'  },
      { label: 'Values Alignment',  value: Math.floor(base * 0.9) + 8,  color: 'bg-emerald-500' },
      { label: 'Present Moment',    value: Math.floor(base * 0.7) + 15, color: 'bg-sky-500'     },
    ];
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className={`w-12 h-12 border-4 ${themeClasses.border} border-t-transparent rounded-full animate-spin`} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <section className={`${themeClasses.secondary} rounded-[2.5rem] p-10 text-slate-800 shadow-2xl relative overflow-hidden transition-colors duration-500`}>
        <div className={`absolute top-0 right-0 w-96 h-96 ${themeClasses.primary} opacity-10 rounded-full -mr-48 -mt-48 blur-3xl`} />
        ...
        <section className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-8">
              <div className={`w-32 h-32 rounded-full border-8 ${themeClasses.secondary} flex flex-col items-center justify-center bg-white shadow-inner shrink-0`}>
                <span className={`text-3xl font-black ${themeClasses.text}`}>{valueStepsCount}</span>
                <span className="text-[8px] font-black text-slate-400 uppercase text-center mt-1 leading-tight">
                  Steps<br />Taken
                </span>
              </div>
              <div className="flex-1 space-y-4 text-center md:text-left">
                ...
                <NavLink
                  to="/values-log"
                  className={`inline-block px-6 py-2.5 ${themeClasses.primary} text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg ${themeClasses.shadow}`}
                >
                  Record Today's Step
                </NavLink>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <section className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 flex flex-col justify-between">
          <div>
            ...
            <div className={`text-sm text-slate-600 italic leading-relaxed ${themeClasses.secondary} p-4 rounded-2xl border ${themeClasses.border} min-h-[100px] flex items-center`}>
              "{dailyInsight}"
            </div>
          </div>
          <div className="mt-4">
        ...
        {/* My Compass / Values */}
        <section className={`${themeClasses.secondary} rounded-3xl p-8 text-slate-800 relative overflow-hidden flex flex-col justify-between transition-colors duration-500 shadow-sm border ${themeClasses.border}`}>
          <div className={`absolute top-0 right-0 w-64 h-64 ${themeClasses.primary} opacity-10 rounded-full -mr-24 -mt-24 blur-3xl`} />
          ...
          <div className="flex flex-wrap gap-3 mt-6 relative z-10">
            {userValues.map((val, idx) => (
              ...
              >
                <i className={`fa-solid ${val.icon} ${val.color} text-[10px]`} />
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-700">{val.name}</span>
              </div>
            ))}
          </div>
          <NavLink
