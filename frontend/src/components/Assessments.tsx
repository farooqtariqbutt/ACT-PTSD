...
const Assessments: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const weeklyLimit =
          freqMap[
            fetchedProfile?.sessionFrequency ||
        ...
        const completedThisWeek = sessionHistory.filter((s: any) => {
          return (
            s.status === "COMPLETED" && new Date(s.timestamp) >= startOfWeek
          );
        }).length;
        ...
        const currentPref =
          fetchedProfile?.schedulePreference ||
        ...
        if (
          fetchedProfile?.currentClinicalSnapshot?.pcl5Total > 0 &&
          !isPostAssessment
        ) {
          setStep("education");
          setLoading(false);
  ...
  const handleFinalSubmit = async (isEarlyExit: boolean) => {
    ...
    try {
      ...
      if (!isEarlyExit && redFlagTemplate) {
        ...
        const totalRiskScore = redFlagItems.reduce(
        ...
        await saveAssessment({
          ...
          interpretation:
            totalRiskScore > 0
              ? "Safety Risk Indicated"
              : "No Safety Risk Reported",
          phase: isPostAssessment ? "POST" : "PRE",
          items: redFlagItems,
        });
      }
      ...
      if (isPostAssessment) {
        ...
        const firstAllowed =
          profile?.prescribedSessions && profile.prescribedSessions.length > 0
  ...
  // ── 8. Render Helpers ─────────────────────────────────────────────────────
  const stepOrder1: AssessmentStep[] = [
    "intro",
    "mood",
    "demographics",
    "traumaHistory",
  ...
  const stepOrder2: AssessmentStep[] = [
    "mood",
    "traumaHistory",
    "pdeq",
    "ders",
  ...
  const isTherapistApproved =
    profile?.prescribedSessions && profile.prescribedSessions.length > 0;
  const hasScheduleSet = !!(
    profile?.schedulePreference || user?.schedulePreference
  ); // <-- NEW
  ...
  const getDynamicButtonLabel = () => {
    switch (step) {
      ...
      case "pcl5":
        return `Next: ${assessmentPrefix}-Assessment 1 Summary`;
      case "ders":
        return "Continue to Next Section";
      case "aaq":
        return "Continue to Next Section";
    ...
    return (
      <div className="space-y-12">
        <div className="text-center">
          <span
            className={`text-xs font-black uppercase tracking-widest ${themeClasses.text} ${themeClasses.secondary} px-3 py-1 rounded-full`}
          >
            ...
            <p className="text-slate-600 mt-6 font-medium max-w-2xl mx-auto leading-relaxed text-lg">
              {instructions.split("*").map((part, i) =>
                i % 2 === 1 ? (
                  <em
                    key={i}
                    className={`font-bold ${themeClasses.text} not-italic`}
                  >
                    {part}
                  </em>
                ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <p className="text-slate-500 font-medium leading-relaxed mb-8">
              Are you sure you want to quit the Assessments this time? It will
              lose the data you've entered so far.
            </p>
      ...
      <div className="bg-white rounded-[3rem] p-10 md:p-16 border border-slate-200 shadow-2xl relative overflow-hidden">
        ...
        {step !== "education" && (
          <div className="absolute top-0 left-0 w-full h-1.5 bg-slate-100">
            <div
              ...
              style={{
                width: `${
                  (currentStepOrder.indexOf(step) /
                    (currentStepOrder.length - 1)) *
                  100
                }%`,
              }}
            />
        ...
        {step === "intro" && (
          <div className="text-center space-y-8 py-10">
            <div
              className={`w-24 h-24 ${themeClasses.secondary} ${themeClasses.text} rounded-[2rem] flex items-center justify-center text-4xl mx-auto shadow-inner`}
            >
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 max-w-lg mx-auto leading-relaxed">
                Complete the two-part assessment process to{" "}
                {isPostAssessment
                  ? "evaluate your progress and recovery"
                  : "receive a clinical evaluation and matching with a specialized therapist"}
                .
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left max-w-2xl mx-auto">
              {[
                {
                  label: `${assessmentPrefix}-Assessment 1`,
                  desc: "Demographics, Trauma History & PCL-5.",
                },
                {
                  label: `${assessmentPrefix}-Assessment 2`,
                  desc: "Dissociation, Emotion Regulation & Safety.",
                },
                {
                  label: "Clinical Profile",
                  desc: "Detailed diagnostic mapping.",
                },
                {
                  label: isPostAssessment
                    ? "Progress Report"
                    : "Therapist Match",
                  desc: isPostAssessment
                    ? "Compare pre and post scores."
                    : "Specialized clinical pairing.",
                },
              ].map((item) => (
                <div
                  key={item.label}
                ...
                >
                  <i
                    className={`fa-solid fa-check-double ${themeClasses.text} mt-1`}
                  ></i>
                  <div>
          ...
          <div className="space-y-12">
            <div className="text-center">
              <span
                className={`text-xs font-black uppercase tracking-widest ${themeClasses.text} ${themeClasses.secondary} px-3 py-1 rounded-full`}
              >
                {assessmentPrefix}-Assessment {activeAssessment} - Phase 1 of{" "}
                {currentStepOrder.length - 1}
              </span>
                ...
                >
                  ...
                  <span
                    className={`text-[10px] font-black uppercase ${
                      mood === v ? "text-white opacity-80" : "opacity-60"
                    }`}
                  >
                    Level {v}
                  </span>
            ...
            >
              {activeAssessment === 1
                ? `Next: ${assessmentPrefix}-Assessment 1 Summary`
                : `Next: ${assessmentPrefix}-Assessment 2 Summary`}
            </button>
          </div>
        )}
        ...
        {step === "demographics" && (
          <div className="space-y-10">
            ...
            <button
              disabled={
                !demoData.age || parseInt(demoData.age) <= 0 || !demoData.gender
              }
              onClick={nextStep}
          ...
          <div className="space-y-10">
            <div className="text-center">
              <span
                className={`text-xs font-black uppercase tracking-widest ${themeClasses.text} ${themeClasses.secondary} px-3 py-1 rounded-full`}
              >
                {assessmentPrefix}-Assessment {activeAssessment} - Phase{" "}
                {activeAssessment === 1 ? "3 of 5" : "2 of 6"}
              </span>
            ...
            <div className="space-y-6">
              {[
                ...
                {
                  label: "Natural Disaster (Flood / Earthquake)",
                  key: "naturalDisaster",
                },
                {
                  label: "War / Political Violence",
                  key: "warPoliticalViolence",
                },
                {
                ...
                {
                  label: "Witnessing Violence at home / in the community",
                  key: "witnessedViolence",
                },
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    {(traumaData as any)[item.key].experienced && (
                      <div className="w-full md:w-64 animate-in zoom-in-95">
                        <label
                          className={`text-[8px] font-black ${themeClasses.accent} uppercase tracking-widest ml-1`}
                        >
                        ...
                        <input
                          ...
                          className={`w-full p-3 bg-white border rounded-xl text-xs font-bold outline-none disabled:bg-slate-50 transition-all ${
                            parseInt((traumaData as any)[item.key].age) >
                            parseInt(demoData.age)
            ...
            <button
              disabled={Object.values(traumaData).some(
                (item: any) =>
                  item.experienced &&
                  (!item.age ||
                    parseInt(item.age) <= 0 ||
                    parseInt(item.age) > parseInt(demoData.age))
              )}
              onClick={nextStep}
          ...
          <div className="space-y-10">
            <div className="text-center">
              ...
              <p className="text-slate-500 mt-4 font-medium max-w-2xl mx-auto leading-relaxed">
                Please read each question carefully. You may select more than
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="py-6 pr-4 text-xs font-black text-slate-400 uppercase tracking-widest">
                      Question
                    </th>
                    <th className="py-6 px-2 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Yes
                    </th>
                    <th className="py-6 px-2 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      No
                    ...
                    <th className="py-6 pl-2 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Ever in Your Life
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="py-6 px-2 text-center">
                        <button
                          ...
                          className={`w-6 h-6 rounded-full border-2 transition-all flex items-center justify-center mx-auto ${
                            redFlagData[qIdx]?.hasFlag === false
                        ...
                        <button
                          ...
                          onClick={() =>
                            setRedFlagData((prev) => ({
                              ...
                              [qIdx]: {
                                ...(prev[qIdx] || {}),
                                pastMonth: !prev[qIdx].pastMonth,
                              },
                            }))
                          }
                          className={`w-8 h-8 rounded-lg border-2 transition-all flex items-center justify-center mx-auto disabled:opacity-20 ${
                            redFlagData[qIdx]?.pastMonth
                              ? "bg-rose-600 border-rose-600 text-white shadow-lg"
                              : "bg-white border-slate-200 hover:border-rose-300"
                          }`}
                        >
            ...
            <div className="p-6 bg-rose-50 rounded-3xl border border-rose-100 flex gap-4 items-start">
              ...
              <p className="text-xs text-rose-700 font-medium leading-relaxed">
                Your safety is our priority. If you select any of these flags,
                your clinician will be notified immediately for a follow-up
                discussion.
              </p>
            </div>
          </div>
                ...
                (step === "pdeq" && pdeqScores.includes(-1)) ||
                  ...
                  (step === "redFlags" &&
                    ...
                    (Object.keys(redFlagData).length !==
                      redFlagTemplate.questions.length ||
                      Object.values(redFlagData).some(
                        (d: any) =>
                          d.hasFlag === null ||
                          (d.hasFlag === true &&
                            !d.rightNow &&
        ...
        {step === "summary1" && (
          <div className="text-center space-y-10 py-10">
            <div className="space-y-4">
              <h2 className="text-4xl font-black text-slate-800 tracking-tight">
                {assessmentPrefix}-Assessment 1 Complete
              </h2>
              <p className="text-slate-500">
                {isPostAssessment ? "Post-program" : "Initial"} evaluation for{" "}
                {demoData.name || "Alex"}.
              </p>
            </div>
              ...
              <div className="bg-slate-50 rounded-[2.5rem] p-10 border border-slate-100 shadow-sm text-left">
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex justify-between items-center">
                  {user?.role !== UserRole.CLIENT && (
                    <span className={`${themeClasses.text} font-black`}>
                      {pcl5Score} / 80
                    </span>
                  )}
                </h4>
                ...
                <div className="mb-8">
                  <div
                    className={`inline-flex items-center gap-2 px-6 py-3 rounded-2xl border-2 font-black text-sm uppercase tracking-widest ${
                      getPCL5Interpretation(pcl5Score).bg
                    } ${getPCL5Interpretation(pcl5Score).color} ${
                      getPCL5Interpretation(pcl5Score).border
                    }`}
                  >
                ...
                {!isPcl5High ? (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-3xl">
                      <p className="text-emerald-800 font-medium leading-relaxed">
                        Thank you for completing Assessment 1! Based on your
                        responses, your results are in a normal, healthy range.
                        To continue taking great care of your mental well-being,
                        we recommend:
                      </p>
                      <ul className="mt-4 space-y-3 text-emerald-700 font-bold">
                        <li className="flex items-center gap-3">
                          <span>💤</span> Prioritising proper sleep
                        </li>
                        <li className="flex items-center gap-3">
                          <span>🚶‍♂️</span> Going for morning walks
                        </li>
                        <li className="flex items-center gap-3">
                          <span>😮‍💨</span> Practising deep breathing when you
                          feel tense
                        </li>
                      </ul>
                    </div>
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    >
                      Begin {assessmentPrefix}-Assessment 2{" "}
        ...
        {step === "summary2" && (
          <div className="text-center space-y-10 py-10">
            <div className="space-y-4">
              ...
              <p className="text-slate-500">
                Full Evaluation for {demoData.name || "Alex"}.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto text-left">
              ...
              <div className="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                ...
                <div className="mb-8">
                  <div
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-black text-xs uppercase tracking-widest ${
                      getPCL5Interpretation(pcl5Score).bg
                    } ${getPCL5Interpretation(pcl5Score).color} ${
                      getPCL5Interpretation(pcl5Score).border
                    }`}
                ...
                <div className="space-y-4">
                  {redFlagTemplate.questions.map((q, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="bg-white p-4 rounded-2xl border border-rose-100 flex flex-col md:flex-row md:items-center justify-between gap-4"
                      >
                        <div className="space-y-1">
                          <p className="text-sm font-bold text-slate-800">
                            {q.text}
                          ...
                          <span
                            className={`inline-block px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${severity.color}`}
                          >
                  ...
                  {Object.values(redFlagData).every(
                    (d: any) => d.hasFlag !== true
                  ) && (
                    <p className="text-sm font-medium text-slate-500 italic">
                      No safety red flags reported.
                    </p>
                  )}
                </div>
              </div>
